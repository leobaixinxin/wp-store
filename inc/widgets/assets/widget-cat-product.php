...
if(wp_store_woocommerce_available()):
  add_action('widgets_init', 'wp_store_register_cat_product_widget');

  function wp_store_register_cat_product_widget(){ //functions start from here
    register_widget('wp_store_cat_product');
  }

  class wp_store_cat_product extends WP_Widget {
  /**
  ...
  */
  public function __construct() {
    parent::__construct(
      'wp_store_cat_product', __('WP Store: WC Product By Category','wp-store'), array(
        'description' => __('This widgets show the Category Image,Description and Product of that Category', 'wp-store')
        )
      );
  }
    ...
    if($instance){
      ...
      $product_args =  array(
        ...
        ?>
            ...
            echo $before_widget;
            ?>
            <div class="feature-cat-product-wrap <?php echo esc_attr($product_alignment);?>">
              <?php 
              ...
              <a href="<?php echo esc_url( $terms_link ); ?>">
                <div class="feature-cat-image">
                  ...
                  if (!empty($thumbnail_id)) {
                    ...
                    echo '<img src="' . esc_url($image[0]) . '" alt="'.esc_attr__('Category Image','wp-store').'"  />';
                  }
                  else{ 
                    ?>

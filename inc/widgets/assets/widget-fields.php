 ...
 * @package WP_Store
 */
function wp_store_widgets_show_widget_field($instance = '', $widget_field = '', $athm_field_value = '') {
    
    extract($widget_field);

    switch ($wp_store_widgets_field_type) {

        // Standard text field
        case 'text' :
        ?>
        <p>
            <label for="<?php echo esc_attr($instance->get_field_id($wp_store_widgets_name)); ?>"><?php echo $wp_store_widgets_title; ?>:</label>
            <input class="widefat" id="<?php echo esc_attr($instance->get_field_id($wp_store_widgets_name)); ?>" name="<?php echo esc_attr($instance->get_field_name($wp_store_widgets_name)); ?>" type="text" value="<?php echo esc_attr($athm_field_value) ; ?>" />

            <?php if (isset($wp_store_widgets_description)) { ?>
            <br />
            <small><?php echo $wp_store_widgets_description; ?></small>
            <?php } ?>
        </p>
        <?php
        break;
        ...
        // Textarea field
            ...
            <p>
                <label for="<?php echo esc_attr($instance->get_field_id($wp_store_widgets_name)); ?>"><?php echo $wp_store_widgets_title; ?>:</label>
                <textarea class="widefat" rows="<?php echo esc_attr($wp_store_widgets_row); ?>" id="<?php echo esc_attr($instance->get_field_id($wp_store_widgets_name)); ?>" name="<?php echo esc_attr($instance->get_field_name($wp_store_widgets_name)); ?>"><?php echo esc_attr($athm_field_value); ?></textarea>
            </p>
            <?php
            break;
        ...
        // Checkbox field
            ...
            ?>
            <p>
                <input id="<?php echo esc_attr($instance->get_field_id($wp_store_widgets_name)); ?>" name="<?php echo esc_attr($instance->get_field_name($wp_store_widgets_name)); ?>" type="checkbox" value="1" <?php checked('1', $athm_field_value); ?>/>
                <label for="<?php echo esc_attr($instance->get_field_id($wp_store_widgets_name)); ?>"><?php echo $wp_store_widgets_title; ?></label>
        ...
        // Radio fields
            ...
            <p>
                <?php
                echo $wp_store_widgets_title;
                echo '<br />';
                foreach ($wp_store_widgets_field_options as $athm_option_name => $athm_option_title) {
                    ?>
                    <input id="<?php echo $instance->get_field_id($athm_option_name); ?>" name="<?php echo esc_attr($instance->get_field_name($wp_store_widgets_name)); ?>" type="radio" value="<?php echo $athm_option_name; ?>" <?php checked($athm_option_name, $athm_field_value); ?> />
        ...
        // Select field
                ...
                <p>
                    <label for="<?php echo esc_attr($instance->get_field_id($wp_store_widgets_name)); ?>"><?php echo $wp_store_widgets_title; ?>:</label>
                    <select name="<?php echo esc_attr($instance->get_field_name($wp_store_widgets_name)); ?>" id="<?php echo esc_attr($instance->get_field_id($wp_store_widgets_name)); ?>" class="widefat">
                        <?php foreach ($wp_store_widgets_field_options as $athm_option_name => $athm_option_title) { ?>
                ...
                ?>
                <p>
                    <label for="<?php echo esc_attr($instance->get_field_id($wp_store_widgets_name)); ?>"><?php echo $wp_store_widgets_title; ?>:</label><br />
                    <input name="<?php echo esc_attr($instance->get_field_name($wp_store_widgets_name)); ?>" type="number" step="4" min="4" id="<?php echo esc_attr($instance->get_field_id($wp_store_widgets_name)); ?>" value="<?php echo esc_attr($athm_field_value); ?>" class="small-text" />
                ...
                $output = '';
                $id = esc_attr($instance->get_field_id($wp_store_widgets_name));
                $class = '';
                $int = '';
                $value = esc_url($athm_field_value);
                $name = esc_attr($instance->get_field_name($wp_store_widgets_name));
                ...
                $output .= '<label for="'.$id.'">'.$wp_store_widgets_title.'</label><br/>';
                $output .= '<input id="' . $id . '" class="upload' . $class . '" type="text" name="' . $name . '" value="' . $value . '" placeholder="' . __('No file chosen', 'wp-store') . '" />' . "\n";
                if (function_exists('wp_enqueue_media')) {
                    if (( $value == '')) {
                        $output .= '<input id="upload-' . $id . '" class="upload-button-wdgt button" type="button" value="' . __('Upload', 'wp-store') . '" />' . "\n";
                    } else {
                        $output .= '<input id="remove-' . $id . '" class="remove-file button" type="button" value="' . __('Remove', 'wp-store') . '" />' . "\n";
                    }
                } else {
                    $output .= '<p><i>' . __('Upgrade your version of WordPress for full media support.', 'wp-store') . '</i></p>';
                }
                ...
                <p>
                    ...
                    <input class="hidden-icon-input" name="<?php echo esc_attr($instance->get_field_name($wp_store_widgets_name)); ?>" type="hidden" id="<?php echo esc_attr($instance->get_field_id($wp_store_widgets_name)); ?>" value="<?php echo esc_attr($athm_field_value); ?>" />
                ...
                <div id="ed-font-awesome-list">
                    <ul class="ed-font-group">
        ...
        function wp_store_widgets_updated_field_value($widget_field, $new_field_value) {

            extract($widget_field);

    // Allow only integers in number fields
            if ($wp_store_widgets_field_type == 'number') {
                return absint($new_field_value);

        // Allow some tags in textareas
            } elseif ($wp_store_widgets_field_type == 'textarea') {
        // Check if field array specifed allowed tags
                if (!isset($wp_store_widgets_allowed_tags)) {
            // If not, fallback to default tags
                    $wp_store_widgets_allowed_tags = '<p><strong><em><a>';
                }

                return strip_tags($new_field_value, $wp_store_widgets_allowed_tags);

        // No allowed tags for all other fields
            } elseif ($wp_store_widgets_field_type == 'iframe_textarea') {
        // Check if field array specifed allowed tags
                if (!isset($wp_store_widgets_allowed_tags)) {
            // If not, fallback to default tags
                    $wp_store_widgets_allowed_tags = '<iframe>';
                }
        ...
        // No allowed tags for all other fields
            ...
            elseif ($wp_store_widgets_field_type == 'url') {
                return esc_url_raw($new_field_value);
            }
            elseif ($wp_store_widgets_field_type == 'title') {
...
if(is_admin()):
     ...
     */
    function wp_store_widgets_media_scripts($hook) {
        if ( 'customize.php' == $hook || 'widgets.php' == $hook ) {
           ...
           wp_register_script('of-media-uploader', get_template_directory_uri() . '/inc/widgets/js/media-uploader.js', array('jquery'), 1.70);
            ...
            wp_localize_script('of-media-uploader', 'optionsframework_l10n', array(
                'upload' => __('Upload', 'wp-store'),
                'remove' => __('Remove', 'wp-store')
            ));
            wp_enqueue_style( 'wp-store-font-awesome', get_template_directory_uri() . '/css/font-awesome.min.css' );
            wp_enqueue_style( 'wp-store-admin-control', get_template_directory_uri() . '/inc/css/admin-control.css' );
    ...
    add_action('admin_enqueue_scripts', 'wp_store_widgets_media_scripts');
endif;